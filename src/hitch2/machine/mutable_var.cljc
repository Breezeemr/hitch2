    ...
    (-apply-command [_ machine-selector graph-value node children parents command]
      (case (nth command 0)
        :set-value (let [[_ val] command]
                     (-> node
                         (assoc :state val)
                         (update :project-values assoc (mutable-var (:var-name machine-selector)) val)))
        :clear (-> node
                   (assoc :state NOT-FOUND-SENTINEL)
                   (update :project-values assoc (mutable-var (:var-name machine-selector)) NOT-FOUND-SENTINEL))))))
